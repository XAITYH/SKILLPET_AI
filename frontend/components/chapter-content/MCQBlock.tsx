...
import { CircleCheck, Check, X } from "lucide-react";
...
export function MCQBlock({ content }: MCQBlockProps) {
  ...
  return (
    <div className="rounded-2xl bg-[#111] border border-white/5 p-6 md:p-8">
      <div className="flex items-center gap-3 mb-6">
        ...
        <h3 className="text-lg font-bold text-white">Multiple Choice</h3>
      </div>
      ...
      <div className="grid grid-cols-1 gap-3 mb-6">
        {content.options.map((option, idx) => (
          ...
          >
            <span className="mr-2 text-zinc-500">{String.fromCharCode(65 + idx)}.</span>
            {option}
          </button>
        ))}
      </div>
      ...
      {!submitted && selectedIndex !== null && (
        <button
          ...
          className="px-4 py-2 rounded-lg bg-cyan-600 hover:bg-cyan-500 text-white text-sm font-semibold transition-colors cursor-pointer"
        >
          Submit Answer
        </button>
      )}
